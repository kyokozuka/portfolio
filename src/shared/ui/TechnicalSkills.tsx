...
export default function TechnicalSkills({ currentLang = "en" }: TechnicalSkillsProps) {
  ...
  return (
    <ul className="grid md:grid-cols-2 gap-8 list-none m-0 p-0">
      {skillCategories.map((category) => (
        <li key={category.name}>
          <article className="bg-white/10 backdrop-blur-md rounded-xl p-6 shadow-xl border border-white/20 hover:shadow-2xl transition-all duration-300 h-full">
            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-3 drop-shadow-lg">
              <div className={`w-4 h-4 rounded-full bg-${category.color}-400 shadow-lg`} aria-hidden="true"></div>
              {currentCategoryTranslations[category.name as keyof typeof currentCategoryTranslations] || category.name}
            </h3>
            <ul className="space-y-4 list-none m-0 p-0">
              {category.skills.map((skill) => (
                <li key={skill.name} className="flex items-center justify-between p-4 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 hover:bg-white/10 hover:shadow-lg transition-all duration-300 hover:scale-[1.02]">
                  <div className="flex items-center gap-4">
                    ...
                    <dl className="m-0">
                      ...
                      <dd className="text-sm text-gray-300 m-0">{skill.experience}</dd>
                      <dd className="text-xs text-gray-400 mt-1 m-0">Source: {skill.source}</dd>
                    </dl>
                  </div>
