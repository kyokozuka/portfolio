...
上から順に潰せばゴールに到達する、実行順の全タスク。1 Phase = 1 PR を基本とする。詳細な対応関係は上記「ページ別移行タスク」「共有コードの移行マッピング」を参照。

**各 Phase の完了条件（全 Phase 共通）**:

- [ ] `mise run lint` / `type-check` / `build` が通る
- [ ] `mise run test`（Vitest ユニット / TDD）が緑
- [ ] `mise run e2e`（Playwright 回帰）が緑 ※ UI に触れる Phase 2–5 で必須。URL 変更（`/en` `/ja`）に伴うマーカー更新は許容し、それ以外の表示回帰は不可

新規の純粋ロジック（i18n の `isLocale`/`getDictionary`/`slug` 解決等）は **TDD（テスト先行）** で実装する。移行対象のコンポーネント自体は単体テストせず、回帰は E2E で担保する。

なお Phase 0.5（Playwright E2E ベースライン）は導入済み。
...
- [x] `src/shared/` が存在しない（Phase 6 で撤去。`useReveal` は `lib/hooks/` へ移動）
- [ ] `src/features/` が存在しない → **未達**。UI セクション・data・types は RSC ページから再利用中で残存（下記「残作業」）
- [x] `shared → features` のような逆流 import が存在しない（`@/shared` 参照ゼロ）
- [x] 言語切替が全ページで正しく伝播する（`/en` `/ja` で静的生成される）
- [ ] コンテンツの正規ソースが `content/` に一本化されている → **一部**。`content/` パスは確立したが実体は `features/*/data` を再エクスポート中。`resume.yml` の統合は未着手
- [x] `return null` 等のデッドコンポーネント・空バレルが削除されている（`HomeStyles`/`SoftwareStyles`・デッドな `uiuxUtils`・旧 `shared/ui` を削除）
- [x] `software` / `uiux` の個別ルートが `[slug]` に集約されている
- [x] `mise run lint` / `type-check` / `build` が通る
- [x] `mise run test` / `mise run e2e` が緑
- [x] CLAUDE.md・architecture.md がゴール構造に更新されている（Phase 6）

## Phase 6 実績と残作業

**実績（Phase 1–6）**: 全ルートを `app/[lang]/` へ移行、i18n を URL 駆動に是正（`useLanguage` 撤去）、`software`/`uiux` を `[slug]` 集約、`components/`・`lib/`・`content/` を確立、`src/shared/` を完全撤去、デッドコード削除、Vitest + Playwright を整備。全 Phase で build/type-check/lint/test/e2e 緑。

**残作業（follow-up）**:

- `src/features/*/ui` のセクションを `app/**/_components/` へ物理移設し `features/` を撤去
- `content/*` の再エクスポートを実体移設へ置換し、`resume.yml` / `src/data` の重複を SSOT 統合
- `<html lang>` の動的化（現状はルート layout で `en` 固定。`output:export` かつルート layout 単一の制約下での動的化を要調査）
